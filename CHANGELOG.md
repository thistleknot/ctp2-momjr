...
## [4.1.0] — 2026-08-05 — measured against the shipped game, not invented

**Minor.** Gameplay. Requires a NEW game: saves cache compiled SLIC.

### The anchor

**Every number this release touches is derived from something the shipped game
already contains, not chosen.** Four pacing dials were re-anchored in one pass:
research cost, production cost, the calendar, and summon combat strength. In each
case the old value was a hand-picked constant and the new value is a measurement
— of CTP2's own cost bands, of the shipped improvement distribution, of a
handwritten calendar worksheet, of the stock unit roster at the same era.

The point is not that the old numbers were wrong in every case. It is that a
derived number can be re-derived when an input changes, and a chosen one cannot.

### Changed

- **Research costs are projected into per-Age bands read from
  `advance_cost_bands.csv`,** not from base Apolyton Edition `Advance.txt`. AE's
  own bands are pathological (AGE_SIX 18876–55927, AGE_TEN 142969–234743) and
  projecting MoM's costs into them made mid and late research take thousands of
  turns. The rescale preserves each advance's rank order within its Age.

- **Production levers set so generated improvement costs land inside the shipped
  distribution.** `improvement_cost_mult` 215, `shield_cost_mult` 100. Median
  generated cost now matches the shipped median rather than sitting well under it.

- **The calendar is derived from the handwritten calendar-mapping worksheet.**
  `calendar_periods.csv` holds the shape per difficulty tier; the generator
  rewrites the six `TIME_SCALE` blocks in `DiffDB.txt` and derives
  `END_OF_GAME_YEAR` (1900) and `END_OF_GAME_YEAR_EARLY_WARNING` (1800) in
  `Const.txt` from it, instead of those being three unrelated hand-edited numbers.

- **Summon combat strength is measured against stock units of the same era.**
  Attack and defence for the summonable roster are derived from the era-matched
  stock distribution rather than assigned by feel, so a summon is legible as
  "stronger than what you could have built instead" by a stated margin.

### Added (gates)

- Generator-owned calendar: `_write_calendar()` is the single writer for the
  `TIME_SCALE` blocks and the two end-of-game constants. Hand-editing them is now
  a regeneration diff, which is the point.

### Notes

The four dials were previously independent and drifted against each other —
research pace assumed one calendar, the calendar assumed another, and combat
assumed neither. Anchoring all four to shipped data is what makes them movable
together in the next release.
